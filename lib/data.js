...
// Read JSON from data/ directory (not traced by Next.js file tracer)
export const readBuildData = (filename) => {
  const jsonPath = path.join(process.cwd(), `data/${filename}.json`);
  return JSON.parse(fs.readFileSync(jsonPath));
};
...
export const getStatesWithData = () => {
  return readBuildData('locations');
};

export const getTopoJsonUS = () => {
  return readBuildData('states-albers-10m');
};
